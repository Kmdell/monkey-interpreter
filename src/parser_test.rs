...
fn test_let_statement(stmt: &Box<dyn Statement>, test: String) {
    ...
    if !let_stmt.name.value.eq(&test) {
        panic!("let_stmt.value.name not '{}', got='{}'", test, let_stmt.name.value);
    }

    if !let_stmt.name.token_literal().eq(&test) {
        panic!("let_stmt.name.token_literal() not '{}', got='{}'", test, let_stmt.name.token_literal());
    }
}
    ...
    let program = Program {
        statements: vec![
            Box::new(LetStatement {
                token: Token { token_type: LET.to_string(), literal: "let".to_string() },
                name: Identifier {
                    token: Token { token_type: IDENT.to_string(), literal: "myVar".to_string() },
                    value: "myVar".to_string(),
                },
                value: Some(Box::new(Identifier {
                    token: Token { token_type: IDENT.to_string(), literal: "anotherVar".to_string() },
...
fn test_function_literal() {
    ...
    if stmts.len() != 1 {
        panic!("program.statements does not contain {} statements, got={}", 1, stmts.len());
    }

    let stmt = stmts[0].into_expression().unwrap_or_else(|e| panic!("{}", e));

    let function = stmt.expression.as_ref().expect("There is no expression").into_func().unwrap_or_else(|e| panic!("{}", e));

    if function.parameters.len() != 2 {
        panic!("function literal parameters wrong, expected 2, got={}", function.parameters.len());
    }

    test_literal_expression(&function.parameters[0], "x".into());
    test_literal_expression(&function.parameters[1], "y".into());

    let stmts = &function.body.as_ref().expect("There is no body for the function").statements;

    if stmts.len() != 1 {
        panic!("function.body.statements doesnt have 1 statements, got={}", stmts.len());
    }

    let body_stmt = stmts[0].into_expression().unwrap_or_else(|e| panic!("{}", e));

    test_infix_expression(body_stmt.expression.as_ref().expect("There is not expression"), "x".into(), "+".into(), "y".into());
}
...
fn test_function_parameter_parsing() {
    ...
    let tests = vec![
        ...
        Test::new("fn(x, y, z) {}".into(), vec!["x".into(), "y".into(), "z".into()])
    ];
    ...
    for tt in tests {
        ...
        let stmt = program.as_ref().expect("There was no program to parse").statements[0].into_expression().unwrap_or_else(|e| panic!("{}", e));

        let function = stmt.expression.as_ref().expect("There is no expression parsed").into_func().unwrap_or_else(|e| panic!("{}", e));

        if function.parameters.len() != tt.expected_parameters.len() {
            panic!("length of parameters are wrong, expected {}, got={}", tt.expected_parameters.len(), function.parameters.len());
        }
...
fn test_call_expression_parsing() {
    ...
    let exp = stmt.expression.as_ref().expect("There is no expression in the statement").into_call().unwrap_or_else(|e| panic!("{}", e));
    
    test_identifier(exp.function.as_ref().expect("There is not function"), "add".to_string());
...
fn test_call_expression_parameter_parsing() {
    ...
    let tests = vec![
        ...
        Test::new("add(1, 2 * 3, 4 + 5);".into(), "add".into(), vec!["1".into(), "(2 * 3)".into(), "(4 + 5)".into()])
    ];
    ...
    for tt in tests {
        ...
        let stmt = prog.as_ref().expect("There is no program parsed").statements[0].into_expression().unwrap_or_else(|e| panic!("{}", e));
        let exp = stmt.expression.as_ref().expect("There is no expression in the statement").into_call().unwrap_or_else(|e| panic!("{}", e));

        test_identifier(exp.function.as_ref().expect("There is no function"), tt.expected_ident);

        if exp.arguments.len() != tt.expected_args.len() {
            panic!("wrong number of arguments. expected={}, got={}", tt.expected_args.len(), exp.arguments.len());
        }

        for (i, arg) in tt.expected_args.iter().enumerate() {
            if !exp.arguments[i].to_string().eq(arg) {
                panic!("argument {} wrong, expected={}, got={}", i, arg, exp.arguments[i].to_string());
            }
        }
    }
}
...
fn test_let_statements() {
    ...
    for tt in tests {
        ...
        if stmts.len() != 1 {
            panic!("program.statements does not contain 1 statement, got={}", stmts.len());
        }
        ...
        test_literal_expression(val.value.as_ref().expect("There is no value in let"), tt.expected_value)
    }
}
...
fn test_return_statements() {
    ...
    for tt in tests {
        ...
        test_literal_expression(return_stmt.return_value.as_ref().expect("There is no return value"), tt.expected_value);
    }
}
